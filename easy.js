...
const altNumbers = (numArray) => {
  const positive = numArray.filter((x) => x >= 0);
  const negative = numArray.filter((x) => x < 0);

  let result = [];

  const longerArray = positive.length >= negative.length ? positive : negative;
  const shorterArray = positive.length >= negative.length ? negative : positive;
